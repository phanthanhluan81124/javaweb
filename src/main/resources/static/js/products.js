...
function setupDynamicListeners() {
  ...
  document.querySelectorAll('.btn-edit-product').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const product = KFCStore.getProductById(id);
      if (!product) return;
      ...
      document.getElementById('product-spicy').value = product.spicyLevel;
  ...
  document.querySelectorAll('.btn-delete-product').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      showConfirm(
        'Xóa Sản Phẩm', 
        `Bạn có chắc muốn xóa món ăn: "${product.name}" (${product.code}) ra khỏi thực đơn?`,
        () => {
          KFCStore.deleteProduct(id);
          showToast(`Đã xóa thành công: ${product.name}`);
          renderPage();
        }
      );
    });
